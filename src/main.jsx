...
function App() {
  ...
  const todayPlan = dayPlan[todayName] || dayPlan.Tuesday;
  ...
  useEffect(() => {
    ...
    saveState({ sessionDays, flags, applications, technicalCompleted, completedQuestions, mockCompleted, mockAnswer, pipeline, notes, lastSavedAt: nextSavedAt });
    setLastSavedAt(nextSavedAt);
  }, [sessionDays, flags, applications, technicalCompleted, completedQuestions, mockCompleted, mockAnswer, pipeline, notes]);
      ...
      <div className="container">
        ...
        <main className="main-grid">
          ...
          <aside className="side-column">
            <section className="card"><h3>☕ Plan du jour</h3>{todayPlan.map((item, i) => <div className="plan-item" key={item}><b>{i + 1}</b><span>{item}</span></div>)}</section>
